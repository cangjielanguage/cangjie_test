...
class TestSuiteTask:
    ...
    def _search_list(self, base_dir, testlist_paths):
        ...
        temp = base_dir / "testlist_with_excludes"
        if testlist_paths:
            with open(temp, 'w', errors='ignore') as testlist:
                for testlist_path in testlist_paths:
                    with open(testlist_path, 'r', errors='ignore') as cases:
                        content = cases.readlines()
                        for line in content:
                            testlist.write(line)
        include, exclude = read_list(Path(temp))
        try:
            os.remove(temp)
        except Exception as e:
            print("Remove {} failed, maybe remove it manually, {}".format(temp, e))
        cases = []
    ...
    def output_failed(self, result):
        ...
        try:
            ...
            print("WARNING: {} {} {}".format(output["name"], e, e.args))
    ...
    def parallel_run_task(self, process_num):
        ...
        for result in result_queue:
            try:
                ...
                print("{} {} {}".format(e, e.filename, e.filename2))
                continue
        ...
        with open(name, 'w') as f:
            for i in self.cost_time_info:
                if isinstance(self.cost_time_info[i], str):
                    ...
                    f.write('{},{}\n'.format(i, 'error'))
        print('INFO: cost time info write in {}!'.format(name))
    ...
    def run(self, process_num=1, run_time=1):
        ...
        for line in self.gen_summary(print_type, run_time).splitlines():
            logger.info(line)
...
class SingleTask:
    def __init__(self, case, config, running_config):
        ...
        clean = True
        if configs.get_val("debug") or configs.get_val("retry"):
            clean = False
        self.running_config = {
