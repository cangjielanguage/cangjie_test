...
Please check your command : {} """.format(command))
    ...
    if sys.platform == 'linux':  # TODO: may should consider darwin?
        cmd[EXIT_CODE] = 0xff & number  # Transfer to uint8 number, -1 -> 255.
    else:
        cmd[EXIT_CODE] = 0xffffffff & number  # Transfer to uint32 number for windows, -1 -> 4294967295.
    return cmd


def filter_command_line(line: str):
